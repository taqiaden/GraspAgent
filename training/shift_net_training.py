...
from Configurations.config import shift_length
...
from visualiztion import vis_scene, view_npy_open3d, view_shift_pose, view_score

instances_per_sample=3
...
def cumulative_shift_loss(depth,shift_scores,statistics):
    loss = 0
    for j in range(BATCH_SIZE):
        ...
        for k in range(instances_per_sample):
            ...
            shifted_start_point=start_point+((target-start_point)*(shift_elevation_threshold+shift_contact_margin))/np.linalg.norm(target-start_point)
            ...
            distances=np.cross(end_point-shifted_start_point,shifted_start_point-flatten_pc)/np.linalg.norm(end_point-shifted_start_point)
            ...
            # view_score(pc, spatial_mask, 1-shift_scores[j,0][mask].detach().cpu().numpy())
...
def train_(file_ids,learning_rate):
    ...
    for epoch in range(EPOCHS):
        ...
        for i, batch in enumerate(data_loader, 0):
            ...
            '''compute loss'''
            loss=cumulative_shift_loss(depth,shift_scores,statistics)/(BATCH_SIZE*instances_per_sample)
