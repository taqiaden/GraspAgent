...
class TrainGraspGAN:
    ...
    def initialize(self, n_samples=None):
        ...
        self.superior_A_model_moving_rate=MovingRate(gripper_sampling_module_key + '_superior_A_model', decay_rate=0.01,
                                                   initial_val=0.)
        self.moving_anneling_factor = MovingRate(gripper_sampling_module_key + '_anneling_factor', decay_rate=0.01,
                                                 initial_val=0.)
    ...
    def step_discriminator_learning(self, depth, pc, mask, bin_mask, gripper_pose, gripper_pose_ref,
        ...
        if self.sampling_centroid is None:
            ...
            if not altered_objects_elevation:
                ...
                selection_p = (gamma_dive * gamma_pairwise_dist * gamma_rand * gamma_firmness*d_gamma*gamma_small_change*gamma_col) ** (1 / 8)
            else:
                selection_p = (gamma_dive * gamma_pairwise_dist * gamma_rand * gamma_firmness*gamma_small_change*gamma_col) ** (1 / 7)
        ...
        for j in range(len(tracked_indexes)):
            ...
            if A_is_collision_free is not None:
                ...
                if A_is_more_firm and not is_noise_label:
                    ...
                    l3+=  0.5*l1_loss+0.5*(l1_loss**2.0)
                    counter[2]+=1.

                # elif not A_is_more_firm:
                #     l1_loss=(torch.clamp(label.detach().clone() - pred_, 0.) ** 1)
                #
                #     l4+=  0.5*l1_loss+0.5*(l1_loss**2.0)
                #     counter[3]+=1.
