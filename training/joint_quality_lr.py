...
def model_dependent_sampling(pc,model_predictions,spatial_mask,model_max_score,model_score_range,maximum_iterations=1000):
    for i in range(maximum_iterations):
        ...
        selection_probability = 1 - (model_max_score - prediction_).item() / model_score_range
...
def train(batch_size=1,n_samples=None,epochs=1,learning_rate=5e-5):
    ...
    suction_head_statistics = TrainingTracker(name=module_key+'_suction', iterations_per_epoch=len(data_loader), samples_size=len(dataset))
    gripper_head_statistics = TrainingTracker(name=module_key+'_gripper', iterations_per_epoch=len(data_loader), samples_size=len(dataset))
    shift_head_statistics = TrainingTracker(name=module_key+'_shift', iterations_per_epoch=len(data_loader), samples_size=len(dataset))
    ...
    for epoch in range(epochs):
        ...
        for i, batch in enumerate(data_loader, 0):
            ...
            for j in range(b):
                ...
                gripper_head_predictions_=gripper_score[j, 0][mask]

                '''limits'''
                gripper_head_max_score = torch.max(suction_score).item()
                gripper_head_score_range = (gripper_head_max_score - torch.min(suction_score)).item()
                ...
                for k in range(instances_per_sample):
                    '''gripper head'''
                    gripper_target_index=model_dependent_sampling(pc, gripper_head_predictions_, spatial_mask, gripper_head_max_score, gripper_head_score_range)
                    gripper_target_point = pc[gripper_target_index]
                    gripper_prediction_ = suction_head_predictions[gripper_target_index]
                    ...
                    '''suction head'''
                    suction_target_index=model_dependent_sampling(pc, suction_head_predictions, spatial_mask, suction_head_max_score, suction_head_score_range)
            ...
            loss=suction_loss+gripper_loss
            loss.backward()
