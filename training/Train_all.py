...
if __name__ == "__main__":
    while True:
        seeds(time_seed)
        clear_records()
        # train_opening_GAN(n_samples=3, activate_generator_training=True)
        # train_generator(n_samples=2, activate_generator_training=True)
        ...
        for i in range(1000):
            # train_generator(
            try:
                train_opening_GAN(n_samples=100,activate_generator_training=True)
            except Exception as e:
                print(Fore.RED, str(e), Fore.RESET)
        for i in range(1):
            # train_generator(
            try:
                train_generator(n_samples=1000,activate_generator_training=True)
            except Exception as e:
                print(Fore.RED, str(e), Fore.RESET)
