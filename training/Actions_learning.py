...
from models.Grasp_GAN import G, gripper_sampling_module_key, D
...
from visualiztion import view_o3d, view_npy_open3d, dense_grasps_visualization
...
class TrainActionNet:
    def __init__(self, n_samples=None, epochs=1, learning_rate=5e-5):
        ...
        self.gripper_dsicriminator=None
    ...
    def initialize_gripper_sampling_model(self):
        ...
        n_ = GANWrapper(gripper_sampling_module_key,G,D)
        n_.ini_generator(train=False)
        n_.ini_critic(train=False)
        self.gripper_model_N=n_.generator
        self.gripper_dsicriminator=n_.critic
    ...
    def begin(self):
        ...
        for i, batch in enumerate(self.data_loader, 0):
            ...
            # print(pc[objects_mask_numpy].shape)
            # view_npy_open3d(pc[objects_mask_numpy])
            # continue
            ...
            '''generated grasps'''
            # with torch.no_grad():
            pred_gripper_poses, predicted_normal, pred_griper_collision_classifier, suction_quality_classifier, shift_affordance_classifier, background_class = self.actions_net.model(
                depth.clone(), detach_backbone=False)
            ...
            # discrimination_score,_=self.gripper_dsicriminator(depth.clone(),pred_gripper_poses,detach_backbone=True)
            # discrimination_score = discrimination_score.permute(0, 2, 3, 1)[0, :, :, 0][mask]
            # gripper_generator_loss=-discrimination_score[objects_mask].mean()

            '''gripper sampling loss'''
            with torch.no_grad():
                label_gripper_pose= self.gripper_model_N(depth.clone(),detach_backbone=True)

                # gripper_poses2 = pred_gripper_poses[0].permute(1, 2, 0)[mask]  # .cpu().numpy()
                #
                # dense_grasps_visualization(pc, gripper_poses2,
                #                            view_mask=objects_mask,
                #                            sampling_p=None, view_all=False, exclude_collision=True)
                #
                # gripper_poses2 = label_gripper_pose[0].permute(1, 2, 0)[mask]  # .cpu().numpy()
                #
                # dense_grasps_visualization(pc, gripper_poses2,
                #                            view_mask=objects_mask,
                #                            sampling_p=None, view_all=False, exclude_collision=True)
                ...
                # label_collision_score= self.actions_net.model.collision_prediction(depth.clone(),label_gripper_pose)
                ...
                # objects_collision=label_collision_score[0,0][mask]
                objects_collision=pred_griper_collision_classifier[0,0][mask]
                def col_weight(x):
                    return (4*x*(1-x))**7

                weights=(0.00+objects_mask.float())#*col_weight(objects_collision.detach().clone())

                weights/=weights.sum()+1e-3

            label_pose = label_gripper_pose[0].permute(1, 2, 0)[mask]
            prediction_pose = pred_gripper_poses[0].permute(1, 2, 0)[mask]

            label_pose[:, -2]=torch.clamp(label_pose[:,-2],min=0.)
            label_pose[:, -1]=torch.clamp(label_pose[:,-1],max=1.)

            dist_loss=((prediction_pose[:,-2]-label_pose[:,-2])**2.)*weights


            width_loss=((prediction_pose[:,-1]-label_pose[:,-1])**2.)*weights

            beta_loss=((1.00 - F.cosine_similarity(prediction_pose[:,3:5],
                                           label_pose[:,3:5], dim=-1)))*weights

            alpha_loss=((1.00 - F.cosine_similarity(prediction_pose[:,0:3],
                                           label_pose[:,0:3], dim=-1)))*weights
            dist_loss=dist_loss.sum()
            width_loss=width_loss.sum()
            beta_loss=beta_loss.sum()
            alpha_loss=alpha_loss.sum()

            dist_loss=dist_loss*10
            width_loss=width_loss*10
            beta_loss=beta_loss*3
            alpha_loss=alpha_loss*25

            # total_gripper_regression_loss=dist_loss.sum()*1.0+width_loss.sum()*0.16+beta_loss.sum()*0.047+alpha_loss.sum()*0.42
            # total_gripper_regression_loss=dist_loss.sum()+width_loss.sum()*0.16+beta_loss.sum()*0.05+alpha_loss.sum()*2.6
            total_gripper_regression_loss=dist_loss+width_loss+beta_loss+alpha_loss
            ...
            loss=   suction_seal_loss*1  + gripper_collision_loss*1  + shift_appealing_loss*1    +background_detection_loss*30  + suction_sampling_loss*100 +total_gripper_regression_loss
