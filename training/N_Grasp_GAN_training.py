...
class TrainGraspGAN:
    ...
    def prepare_model_wrapper(self):
        ...
        gan.generator_sgd_optimizer(learning_rate=self.learning_rate*10)
    ...
    def step_discriminator_learning(self, depth, pc, mask, bin_mask, gripper_pose, gripper_pose_ref,
                                    is_noise_label,altered_objects_elevation,gamma_col):
        ...
        if self.sampling_centroid is None:
            ...
            if not altered_objects_elevation:
                ...
                selection_p = (gamma_dive * gamma_pairwise_dist * gamma_rand * gamma_firmness*d_gamma*gamma_col) ** (1 / 7)
            else:
                selection_p = (gamma_dive * gamma_pairwise_dist * gamma_rand * gamma_firmness*gamma_col) ** (1 / 6)
        ...
        for t in range(n):
            ...
            if col_loss_counter<max_batch_size:
                l_c, counted, loss_terms_counters,is_curcl_loss = collision_loss(c_, s_, f_, prediction_,
            ...
            if not counted and firm_loss_counter<max_batch_size:
                '''firmness loss'''
                l_f, counted, loss_terms_counters,is_curcl_loss = firmness_loss(c_, s_, f_, prediction_,
