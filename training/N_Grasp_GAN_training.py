...
def firmness_loss(c_, s_, f_, q_, prediction_, label_, prediction_uniqness, dist_factor):
    if sum(s_) == 0 and sum(c_) == 0:
        if (f_[1] - f_[0] > 0.) and q_[1] > 0.5:
            ...
            loss=0
            if relative_firmness<np.random.rand(): return False, None, None, 0, 0, 0
            return True, label_, prediction_, relative_firmness, margin, loss
...
class TrainGraspGAN:
    ...
    def initialize(self, n_samples=None):
        ...
        '''Moving rates'''
        self.moving_collision_rate = MovingRate(N_gripper_sampling_module_key + '_collision', decay_rate=0.01,
                                                initial_val=1.)
        self.moving_firmness = MovingRate(N_gripper_sampling_module_key + '_firmness', decay_rate=0.01, initial_val=0.)
        self.moving_out_of_scope = MovingRate(N_gripper_sampling_module_key + '_out_of_scope', decay_rate=0.01,
                                              initial_val=1.)
        self.relative_sampling_timing = MovingRate(N_gripper_sampling_module_key + '_relative_sampling_timing',
                                                   decay_rate=0.01,
                                                   initial_val=1.)
        self.superior_A_model_moving_rate = MovingRate(N_gripper_sampling_module_key + '_superior_A_model',
                                                       decay_rate=0.001,
                                                       initial_val=0.)
        self.moving_anneling_factor = MovingRate(N_gripper_sampling_module_key + '_anneling_factor', decay_rate=0.1,
                                                 initial_val=0.)

        self.moving_scores_std = MovingRate(N_gripper_sampling_module_key + '_scores_std', decay_rate=0.01,
                                            initial_val=.01)
    ...
    def prepare_model_wrapper(self):
        ...
        gan.critic_adam_optimizer(learning_rate=self.learning_rate, beta1=0., beta2=0.999)
        # gan.critic_sgd_optimizer(learning_rate=self.learning_rate,momentum=0.9, weight_decay_=0.)
        gan.generator_adam_optimizer(learning_rate=self.learning_rate, beta1=0., beta2=0.999)
    ...
    def step_discriminator_learning(self, depth, pc, mask, bin_mask, gripper_pose, gripper_pose_ref,
        ...
        if col_loss_counter + firm_loss_counter == batch_size:
            ...
            # loss=(torch.clamp(inferiors - superiors + 1, 0.)).mean()
    ...
    def get_generator_loss(self, depth, mask, gripper_pose, gripper_pose_ref, tracked_indexes, objects_mask):
        ...
        for j in range(len(tracked_indexes)):
            ...
            if A_is_collision_free is not None:
                if not A_is_collision_free and (counter[1] + counter[3] < batch_size):
            ...
            for j in range(len(tracked_indexes)):
                ...
                if A_is_more_firm is not None:
                    ...
                    if not A_is_more_firm and (counter[1] + counter[3] < batch_size):
                        target_index = tracked_indexes[j][0]
                        label = ref_scores_[target_index]
                        pred_ = pred_scores_[target_index]
                        margin = tracked_indexes[j][4]
