...
from models.gripper_quality import gripper_quality_model_state_path, gripper_quality_net,  gripper_scope_model_state_path
    ...
    def train_one_epoch():
        ...
        for i, batch in enumerate(dloader, 0):
            ...
            for j in range(BATCH_SIZE):
                ...
                '''down_sampling'''
                choices = np.random.choice(pc.shape[0], 50000, replace=False)
                down_sampled_pc = pc[choices, :]
                down_sampled_pc = torch.from_numpy(down_sampled_pc).to('cuda')[None, ...].float()

                down_sampled_poses = target_grasps.squeeze().permute(1, 2, 0)[mask][choices, ...][None, ...].permute(0,2,1)
                with torch.no_grad():
                    labels=source_model(down_sampled_pc,down_sampled_poses)
                prediction_=predictions[j].permute(1,2,0)[mask].squeeze()[choices]
