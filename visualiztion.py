...
def view_features(x,reshape=True,max_iteration=None,view_one_by_one=True,hide_axis=False):
    ...
    if n==1:
        view_image(x[0, 0].cpu().detach().numpy(),hide_axis=hide_axis)
    else:
        if view_one_by_one:
            for i in range(n):
                view_image(x[0, i].cpu().detach().numpy(),hide_axis=hide_axis)
        else:
            a=int(math.sqrt(n))
            c=math.ceil(n/a)
            ...
            for i in range(n):
                ...
                if hide_axis: plt.axis('off')  # Hide the axis labels
            plt.show()
