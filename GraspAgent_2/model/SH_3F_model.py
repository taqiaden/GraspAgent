...
class SHPoseSampler(nn.Module):
    def __init__(self):
        ...
        self.delta = ContextGate_2d_2(in_c1=64, in_c2= 1, out_c=3,
                                      relu_negative_slope=0., activation=nn.SiLU(), use_sin=False,normalize=False,bias=True).to(
            'cuda')

        self.alpha = ContextGate_2d_2(in_c1=64, in_c2= 1+3, out_c=3,
                                      relu_negative_slope=0., activation=nn.SiLU(), use_sin=False,normalize=False,bias=True).to(
            'cuda')
        self.beta = ContextGate_2d_2(in_c1=64, in_c2= 4+3, out_c=2,
                                      relu_negative_slope=0., activation=nn.SiLU(), use_sin=False,normalize=False,bias=True).to(
            'cuda')

        self.gamma = ContextGate_2d_2(in_c1=64, in_c2= 7+2, out_c=2,
                                      relu_negative_slope=0., activation=nn.SiLU(), use_sin=False,normalize=False,bias=True).to(
            'cuda')
        self.fingers_A=ContextGate_2d_2(in_c1=64, in_c2=9+2, out_c=5,
                                          relu_negative_slope=0., activation=nn.SiLU(),use_sin=False,normalize=False,bias=True).to(
            'cuda')

        self.fingers_B=ContextGate_2d_2(in_c1=64, in_c2=11+5, out_c=9,
                                          relu_negative_slope=0., activation=nn.SiLU(),use_sin=False,normalize=False,bias=True).to(
            'cuda')

        self.fingers_C=ContextGate_2d_2(in_c1=64, in_c2=16+9, out_c=4,
                                          relu_negative_slope=0., activation=nn.SiLU(),use_sin=False,normalize=False,bias=True).to(
            'cuda')
...
class SH_G(nn.Module):
    def __init__(self):
        super().__init__()
        self.back_bone = res_unet(in_c=2, Batch_norm=False, Instance_norm=False,
                                  relu_negative_slope=0., activation=nn.SiLU(), IN_affine=False,
                                  activate_skip=False).to('cuda')

        self.back_bone.apply(orthogonal_init_all)
        ...
        self.SH_PoseSampler_.apply(orthogonal_init_all)


        self.grasp_quality_ =ContextGate_2d_2( 64, 29, 1, in_c3=0, relu_negative_slope=0.,
        activation=None, use_sin=False, normalize=False, bias=True, cyclic=False).to('cuda')

        self.grasp_collision_ = ContextGate_2d_2( 64, 29, 1, in_c3=0, relu_negative_slope=0.,
        activation=None, use_sin=False, normalize=False, bias=True, cyclic=False).to('cuda')


        self.grasp_collision2 =ContextGate_2d_2( 64, 29, 1, in_c3=0, relu_negative_slope=0.,
        activation=None, use_sin=False, normalize=False, bias=True, cyclic=False).to('cuda')


        self.grasp_collision3 = ContextGate_2d_2( 64, 29, 1, in_c3=0, relu_negative_slope=0.,
        activation=None, use_sin=False, normalize=False, bias=True, cyclic=False).to('cuda')

        self.grasp_quality_.apply(orthogonal_init_all)
        self.grasp_collision_.apply(orthogonal_init_all)
        self.grasp_collision2.apply(orthogonal_init_all)
        self.grasp_collision3.apply(orthogonal_init_all)
...
class SH_D(nn.Module):
    def __init__(self):
        ...
        # self.att_block = ContextGate_1d_2(in_c1=512 + 3, in_c2=28 + 3 ).to('cuda')

        self.att_block = ContextGate_1d_3(512+3, 28+3,mid_c=64, out_c=1, in_c3=0, relu_negative_slope=0.,
        activation=nn.SiLU(), use_sin=False, normalize=False, bias=False, cyclic=False)

        self.back_bone.apply(orthogonal_init_all)
        self.att_block.apply(orthogonal_init_all)
    ...
    def forward(self, depth, pose, pairs, target_mask, cropped_spheres, latent_vector=None, detach_backbone=False):
        ...
        scores = self.att_block(anchor, pose)
...
class SH_D2(nn.Module):
    def __init__(self):
        ...
        # self.att_block = ContextGate_1d_2(in_c1=64, in_c2=29).to('cuda')

        self.att_block = ContextGate_1d_3(64, 29, out_c=1, in_c3=0, relu_negative_slope=0.,
        activation=nn.SiLU(), use_sin=False, normalize=False, bias=False, cyclic=False).to('cuda')
