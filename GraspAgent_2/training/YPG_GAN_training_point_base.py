...
from GraspAgent_2.model.YPG_GAN_point_base import YPG_G_point_base, YPG_point_base_model_key, YPG_D_point_base
from Online_data_audit.data_tracker import  DataTracker
from check_points.check_point_conventions import GANWrapper
from lib.IO_utils import custom_print
from lib.cuda_utils import cuda_memory_report
from lib.dataset_utils import online_data2
from lib.depth_map import  depth_to_point_clouds, transform_to_camera_frame_torch
...
class TrainGraspGAN:
    ...
    def initialize(self, n_samples=None):
        ...
        self.skip_rate = MovingRate(YPG_point_base_model_key + '_skip_rate',
                                                   decay_rate=0.01,
                                                    initial_val=1.)

        self.gradient_moving_rate = MovingRate(YPG_point_base_model_key + '_gradient', decay_rate=0.01, initial_val=1000.)

        self.moving_collision_rate = MovingRate(YPG_point_base_model_key + '_collision', decay_rate=0.01,
                                                initial_val=1.)
        self.moving_firmness = MovingRate(YPG_point_base_model_key + '_firmness', decay_rate=0.01, initial_val=0.)
        self.moving_out_of_scope = MovingRate(YPG_point_base_model_key + '_out_of_scope', decay_rate=0.01,
                                              initial_val=1.)
        self.relative_sampling_timing = MovingRate(YPG_point_base_model_key + '_relative_sampling_timing',
                                                   decay_rate=0.01,
                                                    initial_val=1.)
        self.superior_A_model_moving_rate = MovingRate(YPG_point_base_model_key + '_superior_A_model',
                                                       decay_rate=0.01,
                                                       initial_val=0.)
        self.moving_anneling_factor = MovingRate(YPG_point_base_model_key + '_anneling_factor', decay_rate=0.01,
                                                 initial_val=0.)

        # self.moving_scores_std = MovingRate(YPG_point_base_model_key + '_scores_std', decay_rate=0.01,
        #                                     initial_val=.01)

        '''initialize statistics records'''

        self.bin_collision_statistics = TrainingTracker(name=YPG_point_base_model_key + '_bin_collision',
                                                        track_label_balance=True)
        self.objects_collision_statistics = TrainingTracker(name=YPG_point_base_model_key + '_objects_collision',
                                                            track_label_balance=True)

        self.background_detector_statistics = TrainingTracker(name=YPG_point_base_model_key + '_background_detector',
                                                              track_label_balance=False)


        self.gripper_sampler_statistics = TrainingTracker(name=YPG_point_base_model_key + '_gripper_sampler',
                                                          track_label_balance=False)

        self.grasp_quality_statistics = TrainingTracker(name=YPG_point_base_model_key + '_grasp_quality',
                                                        track_label_balance=True,decay_rate=0.001)

        self.critic_statistics = TrainingTracker(name=YPG_point_base_model_key + '_critic',
                                                 track_label_balance=False)

        self.data_tracker = DataTracker(name=YPG_point_base_model_key)
    ...
    def prepare_model_wrapper(self):
        '''load  models'''
        gan = GANWrapper(YPG_point_base_model_key, YPG_G_point_base, YPG_D_point_base)
            ...
            for k in range(max_samples_per_image):
                ...
                with torch.no_grad():
                    gripper_pose,grasp_quality,_,grasp_collision = self.gan.generator(
                        pc[None],backbone=self.gan.critic.back_bone, detach_backbone=True)  # [1,7,h,w]
...
if __name__ == "__main__":
    # grasp_quality_statistics = TrainingTracker(name=YPG_point_base_model_key + '_grasp_quality',
    #                                                 track_label_balance=True)
