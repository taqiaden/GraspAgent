...
from GraspAgent_2.model.CH_model import CH_model_key2, CH_D2, CH_G
...
from GraspAgent_2.utils.domain_randomization import add_reflective_blob_noise, add_depth_noise
...
class TrainGraspGAN:
    def __init__(self, args,n_samples=None, epochs=1, learning_rate=5e-5):
        ...
        self.last_pose_center_path=CH_model_key2+'_pose_center'
        ...
        # self.quat_centers=OnlingClustering(key_name=CH_model_key2+'_quat',number_of_centers=20,vector_size=4,decay_rate=0.01,is_quat=True,dist_threshold=0.77)
        # self.fingers_centers=OnlingClustering(key_name=CH_model_key2+'_fingers',number_of_centers=10,vector_size=3,decay_rate=0.01,use_euclidean_dist=True,dist_threshold=0.2)
        # self.transition_centers=OnlingClustering(key_name=CH_model_key2+'_transitions',number_of_centers=5,vector_size=1,decay_rate=0.01,use_euclidean_dist=True,dist_threshold=0.2)
        # inti_centers=torch.tensor([[0, 1,0, 1, 0, 0.5,0.5,0.5,  0.]],device='cuda')
        # self.taxonomies=OnlingClustering(key_name=CH_model_key2+'_taxonomies',number_of_centers=30,vector_size=9,decay_rate=0.01,use_euclidean_dist=True)
        # self.alpha=OnlingClustering(key_name=CH_model_key2+'_alpha',number_of_centers=8,vector_size=3,decay_rate=0.01,use_euclidean_dist=False)
        # self.beta=OnlingClustering(key_name=CH_model_key2+'_beta',number_of_centers=10,vector_size=2,decay_rate=0.01,use_euclidean_dist=False)
        # self.fingers=OnlingClustering(key_name=CH_model_key2+'_fingers',number_of_centers=7,vector_size=3,decay_rate=0.01,use_euclidean_dist=True)
        # self.transition=OnlingClustering(key_name=CH_model_key2+'_transition',number_of_centers=3,vector_size=1,decay_rate=0.01,use_euclidean_dist=True)

    def initialize(self, n_samples=None):
        self.n_samples = n_samples

        '''Moving rates'''
        self.moving_collision_rate = MovingRate(CH_model_key2 + '_collision', decay_rate=0.01,
                                                initial_val=1.)
        self.skip_rate = MovingRate(CH_model_key2 + '_skip_rate',
                                                   decay_rate=0.01,
                                                    initial_val=1.)
        self.superior_A_model_moving_rate = MovingRate(CH_model_key2 + '_superior_A_model',
                                                       decay_rate=0.01,
                                                       initial_val=0.)

        self.G_grad_norm_MR = MovingRate(CH_model_key2 + '_G_grad_norm',
                                                       decay_rate=0.01,
                                                       initial_val=0.)

        self.D_grad_norm_MR = MovingRate(CH_model_key2 + '_D_grad_norm',
                                                       decay_rate=0.01,
                                                       initial_val=0.)
        ...
        '''initialize statistics records'''
        self.bin_collision_statistics = TrainingTracker(name=CH_model_key2 + '_bin_collision',
                                                        track_label_balance=True)
        self.objects_collision_statistics = TrainingTracker(name=CH_model_key2 + '_objects_collision',
                                                            track_label_balance=True)
        self.collision_statistics= TrainingTracker(name=CH_model_key2 + '_collision',
                                                            track_label_balance=True)

        self.gripper_sampler_statistics = TrainingTracker(name=CH_model_key2 + '_gripper_sampler',
                                                          track_label_balance=False)

        self.grasp_quality_statistics = TrainingTracker(name=CH_model_key2 + '_grasp_quality',
                                                        track_label_balance=True,decay_rate=0.001)

        self.critic_statistics = TrainingTracker(name=CH_model_key2 + '_critic',
                                                  track_label_balance=False)

        self.data_tracker = DataTracker(name=CH_model_key2)

    def prepare_model_wrapper(self):
        '''load  models'''
        gan = GANWrapper(CH_model_key2, CH_G, CH_D2)
        ...
        gan.generator_adamW_optimizer(learning_rate=self.args.lr, beta1=0.9, beta2=0.999,weight_decay_=0.,load_check_point=self.args.load_last_optimizer)
    ...
    def step_discriminator(self,cropped_spheres,depth, clean_depth,  gripper_pose, gripper_pose_ref ,pairs,floor_mask,grasp_quality,latent_vector ):
        ...
        for j in range(len(pairs)):
            ...
            if k>0:
                loss+=(hinge_loss(positive=ref_scores[j],negative=generated_scores[j],margin=margin*sim_factor)**k_d)/self.batch_size
                # positive=ref_embedding[j]
                # negative=generated_embedding[j]
            else:
                loss+=(hinge_loss(positive=generated_scores[j],negative=ref_scores[j],margin=margin*sim_factor)**k_d)/self.batch_size
    ...
    def step_generator(self,cropped_spheres,depth,clean_depth,floor_mask,pc,gripper_pose_ref,pairs,latent_vector):
        ...
        for k in range(self.batch_size*4):
            ...
            while True:
                ...
                grasp_success,initial_collision,n_grasp_contact,self_collide,stable_grasp,warning_flag,plan_found  = self.evaluate_grasp(gripper_target_point, gripper_target_pose, view=False,hard_level=(1-self.tou)*hard_level_factor,shake=False,check_kinematics=False)
    ...
    def evaluate_grasp(self, target_point, target_pose, view=False,hard_level=0,shake=True,check_kinematics=True):
        ...
        with torch.no_grad():
            ...
            if view:
                ...
                in_scope, grasp_success, contact_with_obj, contact_with_floor,n_grasp_contact,self_collide,stable_grasp,warning_flag  = self.ch_env.check_graspness(
                    hand_pos=shifted_point, hand_quat=quat, hand_fingers=fingers,
            ...
            if grasp_success is not None:
                if grasp_success and not contact_with_obj and not contact_with_floor:
                    ...
                    return grasp_success ,initial_collision,n_grasp_contact,self_collide,stable_grasp,warning_flag,plan_found

        return False, initial_collision,n_grasp_contact,self_collide,stable_grasp,warning_flag,None
    ...
    def sample_contrastive_pairs(self,pc,  floor_mask, gripper_pose, gripper_pose_ref,
        ...
        # selection_p = gamma_dive*norm_(1-grasp_collision)
        ...
        while t < n:
            ...
            ref_success ,ref_initial_collision,ref_n_grasp_contact,ref_self_collide,stable_ref_grasp,warning_flag,ref_plan_found = self.evaluate_grasp(target_point,target_ref_pose,view=False,shake=True)
            # if ref_success and (not stable_ref_grasp or not ref_plan_found): continue
            ...
            if warning_flag:
                return [], [], None
                # continue
            gen_success, gen_initial_collision, gen_n_grasp_contact, gen_self_collide, stable_gen_grasp,warning_flag,gen_plan_found = self.evaluate_grasp(
                target_point, target_generated_pose, view=False, shake=True)
            # if gen_success and (not stable_gen_grasp or not gen_plan_found): continue
            ...
            # dist_to_centroid = (1 - F.cosine_similarity(target_generated_pose[3:5], self.sampling_centroid[3:5], dim=-1)) / 2
            # u = dist_to_centroid if not ref_success else 1
            # if np.random.random() < u**0.5:
            #     gen_success, gen_initial_collision, gen_n_grasp_contact, gen_self_collide, stable_gen_grasp,warning_flag = self.evaluate_grasp(
            #         target_point, target_generated_pose, view=False, shake=True)
            #     if gen_success  and not stable_gen_grasp: continue
            #     if warning_flag: continue
            #
            # else: continue
            ...
            if ref_success and gen_success:
                ...
                if abs(ref_transition-gen_transition)>0.15:
            ...
            if len(d_pairs)<self.batch_size and not (ref_success and gen_success):
                # if len(d_pairs)==1 and (gen_success or margin<1) and d_pairs[0][1]<0:
                #     pass
                # else:
    ...
    def step(self,i):
        ...
        pc, floor_mask = self.ch_env.depth_to_pointcloud(depth.cpu().numpy(), self.ch_env.intr, self.ch_env.extr)
        pc = torch.from_numpy(pc).cuda()
        ...
        for k in range(self.iter_per_scene):
            ...
            if len(g_pairs) == self.batch_size:
                ...
                self.step_generator(g_cropped_spheres,depth,clean_depth, floor_mask, pc, gripper_pose_ref, g_pairs,latent_vector)
                self.print_pairs_info( g_pairs, gripper_pose, gripper_pose_ref)
                print()
            # else:
            #     print()
            #     print('------------------------------------------------step_Policy--------------------------------------------------------')
            #     self.step_generator(None,depth, floor_mask, pc, gripper_pose_ref, g_pairs,latent_vector)
            #     print()
