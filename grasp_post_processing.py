...
def  gripper_processing(index,point_clouds,poses,isvis):
    ...
    print('prediction------------------------------------------------',relative_pose_5)
...
def suction_processing(index,point_data,normals,isvis):
    ...
    suction_xyz, pre_grasp_mat, end_effecter_mat, T, normal = get_suction_pose(index, point_data, normal)
    ...
    if isvis: visualize_suction_pose(suction_xyz, normal.reshape(1, 3) , T, end_effecter_mat,npy=point_data)

    return True,suction_xyz,normal