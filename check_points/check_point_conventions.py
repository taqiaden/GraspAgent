...
class ModelWrapper():
    ...
    def ini_adamW_optimizer(self,params_group=None,learning_rate=None,file_index=None,beta1=0.9,weight_decay_=weight_decay):
        file_name = self.optimizer_name if file_index is None else str(file_index) + self.optimizer_name
        if learning_rate is not None: self.learning_rate=learning_rate
        if params_group is None:
            self.optimizer = torch.optim.AdamW(self.model.parameters(), lr=self.learning_rate, betas=(beta1, 0.999), eps=1e-8,
                                         weight_decay=weight_decay_)
        else:
            self.optimizer = torch.optim.AdamW(params_group, betas=(beta1, 0.999),
                                              eps=1e-8,
                                              weight_decay=weight_decay_)
        self.optimizer = load_opt(self.optimizer, file_name)
        return self.optimizer
...
class GANWrapper():
    ...
    def generator_adam_optimizer(self,param_group=None,learning_rate=None,beta1=0.9,beta2=0.999,weight_decay_=weight_decay):
        ...
        if param_group is None:
            ...
            self.generator_optimizer = torch.optim.Adam(param_group,lr=self.learning_rate,
    ...
    def generator_adamW_optimizer(self,param_group=None,learning_rate=None,beta1=0.9,beta2=0.999,weight_decay_=weight_decay,load_check_point=True):
        ...
        if param_group is None:
            ...
            self.generator_optimizer = torch.optim.AdamW(param_group,lr=self.learning_rate,
                                                        betas=(beta1, beta2), eps=1e-8,
                                                        weight_decay=weight_decay_)
    ...
    def generator_sgd_optimizer(self, param_group=None,learning_rate=None,weight_decay_=weight_decay,momentum=0.0):
        if learning_rate is not None: self.learning_rate = learning_rate
        ...
        if param_group is None:
            ...
            self.generator_optimizer = torch.optim.SGD(param_group, lr=self.learning_rate,
                                                        weight_decay=weight_decay_,momentum=momentum)
        self.generator_optimizer = load_opt(self.generator_optimizer, self.module_key + '_generator_optimizer')
