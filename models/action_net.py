...
class GripperPartSampler(nn.Module):
    def __init__(self):
        ...
        self.beta_decoder = att_res_mlp_LN(in_c1=64, in_c2=3 + 1, out_c=2,
            ...
            'cuda') if decoder_with_LN else att_res_mlp_BN(in_c1=64, in_c2=3, out_c=2,
                                                           relu_negative_slope=gripper_sampler_relu_slope).to('cuda')
        self.dist_width_decoder = att_res_mlp_LN(in_c1=64, in_c2=5 + 1, out_c=2,
                                                 relu_negative_slope=gripper_sampler_relu_slope).to(
            'cuda') if decoder_with_LN else att_res_mlp_BN(in_c1=64, in_c2=5, out_c=2,
                                                           relu_negative_slope=gripper_sampler_relu_slope).to('cuda')
    ...
    def latent_noise_injection(self, approach, beta, dist_width, randomization_factor):
        ...
        beta_noise = torch.randn((beta.shape[0], 2), device='cuda') if decoder_with_LN else torch.randn(
            (beta.shape[0], 2, beta.shape[2], beta.shape[3]), device='cuda')
        beta = beta * (1.0 - randomization_factor) + beta_noise * (randomization_factor)

        width_noise = 1. - torch.rand(size=(beta.shape[0], 1),
                                      device='cuda') ** 2 if decoder_with_LN else 1. - torch.rand(
        ...
        dist_noise = torch.rand(size=(beta.shape[0], 1), device='cuda') ** 2 if decoder_with_LN else torch.rand(
            size=(beta.shape[0], 1, beta.shape[2], beta.shape[3]), device='cuda') ** 2
    ...
    def forward(self, representation_2d, depth, approach_seed=None, clip=False, randomization_factor=0.0,
                dist_width_sigmoid=False):
        approach = approach_seed

        beta = self.beta_decoder(representation_2d, torch.cat([depth, approach], dim=1))
        dist_width = self.dist_width_decoder(representation_2d, torch.cat([depth, approach, beta], dim=1))
...
class SuctionPartSampler(nn.Module):
    def __init__(self):
        ...
        self.decoder = nn.Sequential(
            ...
            nn.LeakyReLU(negative_slope=suction_sampler_relu_slope) if suction_sampler_relu_slope > 0. else nn.ReLU(),
            nn.Linear(16, 3),
        ).to('cuda') if decoder_with_LN else nn.Sequential(
...
class ActionNet(nn.Module):
    def __init__(self):
        super().__init__()
        self.back_bone = res_unet(in_c=1, Batch_norm=not backbone_with_IN, Instance_norm=backbone_with_IN,
        ...
        if decoder_with_LN:
            ...
            self.spatial_inf_IN = nn.InstanceNorm2d(2).to('cuda')
        ...
        self.gripper_collision = att_res_mlp_LN(in_c1=64, in_c2=7 + 1, out_c=2,
            ...
            'cuda') if decoder_with_LN else att_res_mlp_BN(in_c1=64, in_c2=7, out_c=2,
                                                           relu_negative_slope=classification_relu_slope).to('cuda')

        self.suction_quality = att_res_mlp_LN(in_c1=64, in_c2=3, out_c=1,
                                              relu_negative_slope=classification_relu_slope).to(
            'cuda') if decoder_with_LN else att_res_mlp_BN(in_c1=64, in_c2=3, out_c=1,
                                                           relu_negative_slope=classification_relu_slope).to('cuda')

        self.shift_affordance = att_res_mlp_LN(in_c1=64, in_c2=5, out_c=1,
                                               relu_negative_slope=classification_relu_slope).to(
            'cuda') if decoder_with_LN else att_res_mlp_BN(in_c1=64, in_c2=5, out_c=1,
                                                           relu_negative_slope=classification_relu_slope).to('cuda')

        self.background_detector = att_res_mlp_LN(in_c1=64, in_c2=2, out_c=1,
                                                  relu_negative_slope=classification_relu_slope).to(
            'cuda') if decoder_with_LN else att_res_mlp_BN(in_c1=64, in_c2=2, out_c=1,
                                                           relu_negative_slope=classification_relu_slope).to('cuda')
    ...
    def forward(self, depth, alpha=0.0, random_tensor=None, seed=None, detach_backbone=False, clip=False,
                randomization_factor=0.0, dist_width_sigmoid=False):
        '''input standardization'''
        depth = standardize_depth(depth)

        normalized_depth=self.depth_IN(depth)

        '''backbone'''
        if detach_backbone:
            with torch.no_grad():
                features = self.back_bone(normalized_depth)
        else:
            features = self.back_bone(normalized_depth)
        ...
        if decoder_with_LN:
            ...
            normalized_depth = reshape_for_layer_norm(normalized_depth, camera=camera, reverse=False)
        ...
        '''Spatial data'''
        if self.spatial_encoding.shape[0] != normalized_depth.shape[0]:
            self.spatial_encoding = depth_xy_spatial_data(batch_size=normalized_depth.shape[0])
            if decoder_with_LN:
                self.spatial_encoding=self.spatial_inf_IN(self.spatial_encoding)
                self.spatial_encoding = reshape_for_layer_norm(self.spatial_encoding, camera=camera, reverse=False)
        ...
        if self.training:
            ...
            gripper_pose = self.gripper_sampler(features, normalized_depth, approach_seed=gripper_approach_direction, clip=clip,
            ...
            griper_collision_classifier = self.gripper_collision(features,
                                                                 torch.cat([gripper_pose_detached, normalized_depth], dim=1))
        else:
            '''first forward with approach set to normal'''
            gripper_pose_1 = self.gripper_sampler(features, normalized_depth, approach_seed=normal_direction.detach(), clip=clip,
                                                  randomization_factor=randomization_factor)

            #gripper collision head
            gripper_pose_1_detached = gripper_pose_1.detach().clone()
            gripper_pose_1_detached[:, -2:] = torch.clip(gripper_pose_1_detached[:, -2:], 0., 1.)
            griper_collision_classifier_1 = self.gripper_collision(features,
                                                                   torch.cat([gripper_pose_1_detached, normalized_depth], dim=1))

            '''second forward with approach set to vertical'''
            vertical_direction = torch.zeros_like(normal_direction)
            vertical_direction[:, 2] += 1.0
            gripper_pose_2 = self.gripper_sampler(features, normalized_depth, approach_seed=vertical_direction, clip=clip,
                                                  randomization_factor=randomization_factor)

            # gripper collision head
            gripper_pose_2_detached = gripper_pose_2.detach().clone()
            gripper_pose_2_detached[:, -2:] = torch.clip(gripper_pose_2_detached[:, -2:], 0., 1.)
            griper_collision_classifier_2 = self.gripper_collision(features,
                                                                   torch.cat([gripper_pose_2_detached, normalized_depth], dim=1))
            ...
            bad_normal_approach_mask = bad_normal_approach_mask | (
                        collision_mask_2 & collision_mask_1 & (gripper_pose_2[:, -2] > gripper_pose_1[:, -2]))
...
class Critic(nn.Module):
    def __init__(self):
        super().__init__()
        self.back_bone = res_unet(in_c=1, Batch_norm=not critic_backbone_with_IN, Instance_norm=critic_backbone_with_IN,
                                  relu_negative_slope=critic_relu_slope).to('cuda')
        self.dropout=nn.Dropout2d(0.2)
        self.depth_IN=nn.InstanceNorm2d(1).to('cuda')

        self.att_block = att_res_mlp_LN(in_c1=64, in_c2=7 + 1, out_c=1, relu_negative_slope=critic_relu_slope,
    ...
    def forward(self, depth, pose, detach_backbone=False):
        ...
        '''decode'''
        output = self.att_block(features,query)  #+ self.att_block1(features,query) * self.tanh(self.att_block2(features,query))
