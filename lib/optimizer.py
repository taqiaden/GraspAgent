...
def load_opt(optimizer,path):
    try:
        if isinstance(optimizer,torch.optim.Adam) :
            ...
            optimizer=load_optimizer_state(optimizer, path+'_ADAMW'+check_points_extension)
...
def export_optm(optimizer,path):
    if isinstance(optimizer, torch.optim.Adam) :
        ...
        export_model_state(optimizer, path + '_ADAMW')
    elif isinstance(optimizer, torch.optim.RMSprop):
...
def layer_decay_lr(min_lr,max_lr,model,exponent=1):
    parameters = []
    ...
    return parameters
def layer_decayed_optimizer(model_,lr_list=None,lr_=None,decay_rate=0.5,use_RMSprop=False,use_sgd=False,weight_decay=None):
    ...
    if isinstance(model_,list):
        for i,mode_item in enumerate(model_):
            ...
            parameters += layer_decay_lr(min_lr=decay_rate*lr , max_lr=lr, model=mode_item)
    else:
        parameters += layer_decay_lr(min_lr=lr_i, max_lr=lr, model=model_)
...
def linear_decay_lr_(performance_indicator,max_lr,min_lr):
    assert 1>=performance_indicator>=0. and max_lr>min_lr
    k=1-performance_indicator
    range_=max_lr-min_lr
    return min_lr+range_*k

def exponential_decay_lr_(performance_indicator,max_lr,min_lr):
    assert 1>=performance_indicator>=0. and max_lr>min_lr
    lr=max_lr*((min_lr/max_lr)**performance_indicator)
    return lr